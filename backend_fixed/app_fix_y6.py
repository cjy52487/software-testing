...
def register_test():
    try:
        ...
        # ==========【这里缺失密码规则校验，新增下面校验代码】==========
        import re
        # 规则：必须包含大写、小写、数字；长度6‑12位
        # pwd_pattern = re.compile(r'^(?=.*[a-z])(?=.*[A‑Z])(?=.*\d).{6,12}$')
        pwd_pattern = re.compile(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{6,12}$')

        if not pwd_pattern.match(password):
            return jsonify({"status":1003,
                            "msg":"密码必须包含大写字母、小写字母、数字，长度6‑12位"})
        # ---- 新增手机号校验 ----
        import re
        # 手机号：严格11位数字
        phone_pattern = re.compile(r'^\d{11}$')
        if not phone_pattern.match(telephone):
            return jsonify({"status":1004, "msg":"手机号必须是11位数字"})
        # ==========================================================
...
def add_to_cart():
    try:
        ...
        try:
            ...
            return jsonify(status=400, msg="购买数量必须为数字")
        if quantity <= 0:
            return jsonify(status=400, msg="购买数量必须大于0")
        # ==========新增：数量校验 end ==========
...
def remove_from_cart():
    try:
        ...
        # ==========新增：校验购物车条目归属 start ==========
        cart_record = db.session.execute(
            text("SELECT cart_id FROM cart WHERE cart_id=:cid AND user_phone=:uphone"),
            {"cid": cart_id, "uphone": user_phone}
        ).fetchone()
        if not cart_record:
            return jsonify(status=403, msg="无权删除该购物车条目")
        # ==========新增：校验购物车条目归属 end ==========
...
def add_dish():
    try:
        ...
        if request.content_type and 'multipart/form-data' in request.content_type:
            ...
            if price < 0:
                return jsonify(status=400, msg="菜品价格不能为负数")
        ...
        # 新增：查询刚插入的菜品dish_id
        new_dish = db.session.execute(text("""
            SELECT dish_id FROM dish 
            WHERE shop_id=:shop_id AND dish_name=:dish_name
            ORDER BY dish_id DESC LIMIT 1
        """),{"shop_id":shop_id,"dish_name":dish_name}).fetchone()

        new_dish_id = new_dish.dish_id if new_dish else None

        return jsonify(status=200, 
                    msg="菜品添加成功", 
                    image_url=image_url,
                    data={"dish_id": new_dish_id})
...
def update_order_status(order_id):
    try:
        ...
        # ============【新增状态机流转校验】============
        old_checked = verification.checked
        new_checked = checked
        # 业务流转规则：0待接单 →仅允许1已接单；1已接单 →仅允许2已完成
        allow_transfer = {
            0: [1],
            1: [2]
        }
        if new_checked not in allow_transfer.get(old_checked, []):
            return jsonify({"status":400, "msg":"非法的订单状态跳转"})
        # =============================================
